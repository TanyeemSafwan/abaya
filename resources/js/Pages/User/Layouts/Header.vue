...
const navItems = computed(() => [
  ...
  ...(auth.user ? [{ label: 'Order History', route: route('user.history'), routeName: 'user.history' }] : []),
  ...
  <nav class="bg-gradient-animate border-gray-200 dark:bg-gray-900">
    <div class="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto p-4">
      ...
      <div :class="{'hidden': !isMobileMenuOpen, 'block': isMobileMenuOpen}" class="items-center justify-between w-full md:flex md:w-auto md:order-1" id="navbar-user">
        <ul class="flex flex-col md:flex-row flex-wrap items-center justify-center p-4 md:space-x-8 mt-4 md:mt-0 text-white">
          <li v-for="(item, index) in navItems" :key="index" class="my-2 md:my-0 w-full md:w-auto">
            <Link
              :href="item.route"
              :class="[
                'block py-2 px-4 rounded-full transition-all duration-300 ease-in-out text-center',
          ...
          <li class="my-2 w-full md:hidden">
            <Link :href="route('cart.view')" 
              class="flex items-center justify-center py-2 px-4 rounded-full transition-all duration-300 ease-in-out text-center bg-gradient-to-r from-blue-500 to-indigo-600 hover:from-blue-600 hover:to-indigo-700">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 mr-2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 0 0-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 0 0-16.536-1.84M7.5 14.25 5.106 5.272M6 20.25a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Zm12.75 0a.75.75 0 1 1-1.5 0 .75.75 0 0 1 1.5 0Z" />
              </svg>
              Cart ({{ cart.data.count }})
            </Link>
          </li>
          <li v-if="!auth.user" class="my-2 w-full md:hidden">
            <Link :href="route('login')"
              class="block py-2 px-4 rounded-full transition-all duration-300 ease-in-out text-center bg-gradient-to-r from-cyan-400 to-blue-500 hover:from-cyan-500 hover:to-blue-600">
              Login
            </Link>
          </li>
          <li v-if="!auth.user && canRegister" class="my-2 w-full md:hidden">
            <Link :href="route('register')"
              class="block py-2 px-4 rounded-full transition-all duration-300 ease-in-out text-center bg-gradient-to-r from-green-400 to-blue-500 hover:from-green-500 hover:to-blue-600">
              Register
            </Link>
          </li>
        </ul>
